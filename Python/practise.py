...
def archive_log_files(log_directory: Union[str,Path], archive_date: str) -> list[Path]:
    ...
    if not isinstance(log_directory,(str,Path)):
        raise TypeError("Log Directory must be a string or path object")
    ...
    if not log_dir_path.is_dir():
        raise ValueError(f"Diectory does not exist or it is not a directory: {log_dir_path}")
    
    if not isinstance(archive_date,str):
        raise TypeError("archive date must be astring")
    
    parts= archive_date.split('-')

    is_valid_format = (
        len(parts) == 3 and
        len(parts) ==4 and parts[0].isdigit() and
        len(parts) == 2 and parts[1].isdigit() and
